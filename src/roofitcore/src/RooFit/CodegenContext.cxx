...
{
   // The overloads of the dispatched function are declared in CodegenImpl.h. In
   // the ROOT build, the interpreter knows them from the C++ module of the
   // RooFitCodegen library, but in this standalone build there is no such
   // module, so the header has to be included explicitly. It has to happen
   // before the first dispatch, hence here and not only in RooFuncWrapper.
